...
public:
    glm::vec3 ambient = {0.2, 0.2, 0.2}, diffuse = {0.8, 0.8, 0.8}, specular = {0, 0, 0}, emssion = {0, 0, 0.0};
...
private:
    using ParamMatcher = std::unordered_map<std::string_view, auto (*)(Mesh &m, std::istringstream &)->void>;
    const static ParamMatcher obj_param_matcher, mtl_param_matcher;
    MaterialPool mtlPool_;
    static void fileParser_(std::filesystem::path const &path, Mesh &mesh, ParamMatcher const &matcher);
...
public:
    ...
    static Mesh loadMesh(std::filesystem::path const &model); // path to model.obj
